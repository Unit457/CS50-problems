...
void mediumCalc(void) {
    ...
    while (repeat) {
        const int qntNotas = get_int("How many grades are there? ");
        
        float sum = 0, nota;
        printf("Insert the grades' values: \n");
        for (int i = 1; i < qntNotas+1; i++) {
            nota = get_float("Grade %i: ", i);
            sum+=nota;
        }
...
void unitConv(void) {
    ...
    while (repeat) {
        ...
        const float valor = get_float("Insert the value: ");

        switch (option) {
            case 1: {
                const float valorFahr = (valor * 9/5) + 32;
                printf("%.2f °C = %.2f°F\n\n", valor, valorFahr);
                break;
            }
            
            case 2: {
                const float valorCels = (valor - 32) * 5/9;
                printf("%.2f °F = %.2f°C\n\n", valor, valorCels);
                break;
            }

            case 3: {
                const float valorPes = valor * 3.281;
                printf("%.2f meters tall = %.2f foot tall\n\n", valor, valorPes);
                break;
            }

            case 4: {
                const float valorMetros = valor / 3.281;
                printf("%.2f foot tall = %.2f meters tall\n\n", valor, valorMetros);
                break;
            }
            
            case 5: {
                const float valorLibras = valor * 2.205;
                printf("%.2fkg = %.2flbs\n\n", valor, valorLibras);
                break;
            }
        ...
        while (repeatActions) {
            ...
            switch (option) {
                ...
                case 2:
                    ...
                    do {
                        addedMoney = get_float("How much money would you like to deposit to your account?");
                    } while (addedMoney < 0);
