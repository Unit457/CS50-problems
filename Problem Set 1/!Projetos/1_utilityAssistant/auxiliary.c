...
bool ynValidator(const char *value) {
    ...
    if (strcasecmp(value, "y") == 0 || strcasecmp(value, "yes") == 0) return true;
    
    // Se o valor inserido for "n" ou "no" é válido
    else if (strcasecmp(value, "n") == 0 || strcasecmp(value, "no") == 0) return true;
...
bool askYesNo(const char *value) {
    ...
    if (strcasecmp(yn, "y") == 0 || strcasecmp(yn, "yes") == 0) return true;
    else return false;
}
...
void withdrawal(const int values[], int noteQnt[], float *userMoney, float *sumWithdraw, const int numValues) {
    ...
    while (repeatWithdraw) {
        ...
        do {
            ...
            if (note == 0) {
                noteNotFound = repeatWithdraw = false;
            }
